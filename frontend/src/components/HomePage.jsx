...
const HomePage = () => {
  ...
  const fetchTasks = async () => {
    try {
      const response = await axios.get(`${baseURL}/task/viewAllTasks`);
  ...
  const addTask = async () => {
    if (newTask.trim()) {
      try {
        const response = await axios.post(`${baseURL}/task/createTask`, { taskName: newTask, description: '' });
  ...
  const updateTask = async (id, updatedTask) => {
    try {
      const response = await axios.put(`${baseURL}/task/updateTask/${id}`, updatedTask);
  ...
  const deleteTask = async (id) => {
    try {
      await axios.delete(`${baseURL}/task/deleteTask/${id}`);
